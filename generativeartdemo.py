...
def octagon(x_orig, y_orig, side):  # octagon function will return an array of points
...
def main():
    ...
    parser.add_argument("--width", default=1000, type=int)    # sets width
    parser.add_argument("--height", default=1500, type=int)   # sets height
    ...
    parser.add_argument("-bd", "--basedeforms", default=1, type=int)  # how many times the octagon is deformed before it is layered
    parser.add_argument("-fd", "--finaldeforms", default=3, type=int)  # number of times the octagon is deformed as it is layered
    ...
    for octo in range(-int(height * .2), int(height * 1.2), 60):
        ...
        shape = octagon(random.randint(-100, width + 100), octo, random.randint(100, 300))
        ...
        for layr in range(random.randint(minshapes, maxshapes)):
            ...
            cr.fill()
    ims.write_to_png('watercolor.png') #creates a file for the image
